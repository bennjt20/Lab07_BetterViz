...
```{r Graph1 No data appears when using "geom_line"}
ggplot(KACovid, aes(x=date)) + 
  geom_line(aes(y = mask), color = "darkred") + 
  geom_line(aes(y = No_mask), color = "steelblue")
```  
     
```{r Graph2}
ggplot(KACovid, aes(x=date)) + 
  geom_point(aes(y = mask), color = "darkred") + 
  geom_point(aes(y = No_mask), color="steelblue") +
labs(                                
   x = "Date",
   y = "Mask or No Mask",
   title = "Kansas Covid"
...
NOTE: I tried to add a legend but could not get it
  legend(x = "7/31/2020", y = "20", legend = levels(KACovid$mask), col = c(1:3), pch = 16)

```{r Graph3 No data appears using geom line}
  ggplot(data = KACovid, aes(x = date, y = mask))+
  geom_line()
``` 
  
```{r Graph5 No data appers using geom_line}
ggplot(KACovid, aes(x=date)) + 
  geom_line(aes(y=mask))
``` 

```{r Graph6 "geom_point" works for mask} 
ggplot(KACovid, aes(x = date, y = mask)) +
 geom_point()
``` 
